...
function summarizeLocally(transcript, source) {
    ...
    const extractLinks = (text) => Array.from(text.matchAll(urlRe)).map(m => m[0]);
    const extractPhones = (text) => Array.from(text.matchAll(phoneRe)).map(m => m[0].trim());
    ...
    for (const line of lines) {
        ...
        if (lower.match(/job|position|role|opening|hiring/) ||
...
async function captureMessagesFromTab(tabId) {
    // Try sending the message; if there's no receiver, inject the content script and retry.
    return new Promise((resolve, reject) => {
        const sendOnce = () => {
            chrome.tabs.sendMessage(tabId, { type: 'CAPTURE_MESSAGES' }, (resp) => {
